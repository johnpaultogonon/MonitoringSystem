-- Same as supabase/migrations/20260426003000_align_admin_account_modal.sql
-- Paste into Supabase → SQL Editor → Run, if you do not use CLI `db push`.
...
--
-- Password columns (important):
--   CANONICAL (used by the app + Account Management modal): password_plain
--   LEGACY (remove after backfill): "Password" — values are copied into password_plain, then this column is dropped.
...
-- 3) Backfill: id + email (always). Legacy "adminID" / "Password" only if those columns still exist (idempotent re-runs).
...
-- 6) Drop legacy columns — app reads/writes password_plain only (see supabase-events-adapter.js mapAdminAccountRow / updateAdminAccount).
